...
const getPrenotazioneById = (req, res) => {
  ...
  db.query(
    "SELECT * FROM prenotazione WHERE id=$1;",
    [id],
    (error, results) => {
      if (error) {
        throw error;
...
const createPrenotazione = (req, res) => {
  const { code, utente, tratta } = req.body;
  db.query(
    "INSERT INTO prenotazione (code, utente, tratta) VALUES ($1,$2,$3) RETURNING id;",
    [code, utente, tratta],
    (error, results) => {
      if (error) {
        throw error;
...
const updatePrenotazione = (req, res) => {
  ...
  const { code, utente, tratta } = req.body;
  db.query(
    "UPDATE prenotazione SET code=$1, utente=$2, tratta=$3 WHERE id=$4;",
